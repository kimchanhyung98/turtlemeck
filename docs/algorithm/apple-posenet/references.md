...
| 항목 | 내용 |
|---|---|
| 문서 유형 | 공식 자료·원본 저장소·로컬 구현 근거 목록 |
| 적용 상태 | 근거 문서 |
...
| 현재 번들 모델은 MobileNetV1 0.75, stride 16이며 네 output tensor를 가진다 | 로컬 1차 근거 | 번들 `.mlmodel` metadata |
...
- PoseNet 우선·Vision 대체 경로: `Sources/TurtleCore/Inference/PoseDetector.swift`

번들 모델 metadata는 원본 TensorFlow `tfjs-models` PoseNet 저장소와 Core ML converter 저장소를 출처로 가리킨다.
라이선스는 문서의 요약만 믿지 않고 실제 배포 모델과 원본 저장소의 라이선스를 릴리스마다 다시 확인한다.

## 추가·관련 자료

- Vision 2D 대체 경로: [`../apple-body-pose/analysis.md`](../apple-body-pose/analysis.md)
