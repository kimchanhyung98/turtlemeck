...
목표는 Mac 내장 RGB 카메라의 고정된 시점에서 사용자의 중립 자세보다 머리가 몸통 앞쪽으로 이동한 상태가 지속되는지 감지하는 것이다.
...
머리 ROI는 nose, eyes, ears 중 품질을 충족하는 점들의 견고한 중심을 기준으로 만든다. 몸통 ROI는 양쪽 shoulder의 중점을 기준으로 만든다. neck은 검출되면 진단 정보로 보존하지만 필수점은 아니다. ROI 크기는 고정 픽셀이 아니라 shoulder width 같은 신체 기준 길이에 비례시킨다.
