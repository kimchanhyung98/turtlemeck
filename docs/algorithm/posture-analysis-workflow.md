...
수치 임계와 상태 전이 지속 시간은 제품 데이터 검증 전에는 확정하지 않는다. 분석 세션은 최소 15초 간격으로 실행하고, 한 세션에서는 최대 5장의 짧은 이미지 버스트를 사용하며 유효 프레임이 2장 이상일 때 판정한다. 정규화 feature의 기준 ROI와 견고한 통계도 검증 대상이지만, 이 때문에 모델이나 전체 처리 경로를 다시 선택하지는 않는다.
...
| `analysis.md` | local AI CLI 응답(stdout·stderr)을 호출자가 기록한 자세 분석 결과 |

프레임 번호 `{n}`은 캡처 순서에 따라 `1`부터 시작한다. 한 세션은 최대 5장이므로 `001` 같은 자릿수 채우기를 사용하지 않는다. 같은 프레임에서 생성된 파일은 같은 번호를 사용한다.

local AI CLI에는 `debug/{timestamp}`의 `capture-{n}.png`와 `depth-{n}.png`를 입력으로 전달하고, 분석 텍스트만 반환하도록 요청한다. 반환된 응답(stdout·stderr)은 호출자가 `debug/{timestamp}-local`의 `analysis.md`에 기록한다. CLI가 원본 입력 디렉토리를 수정하거나 `debug/` 밖에 임시 파일을 만들도록 요청하지 않는다.
...
분석은 최소 15초 간격의 짧은 버스트 단위로 수행한다. 버스트는 단일 프레임의 우연한 오류를 줄이면서 카메라 사용 시간과 연산량을 제한한다.

1. 분석이 일시 정지되지 않았고 카메라 권한이 있는지 확인한다.
2. 카메라를 준비하고 노출과 초점이 안정되기 전 프레임은 제외한다.
3. 제한된 시간 동안 최대 5장의 프레임을 수집한다.
...
머리 ROI는 nose, eyes, ears 중 품질을 충족하는 점들의 견고한 중심을 기준으로 만든다. 표준 입력의 몸통 ROI는 양쪽 shoulder의 중점 기준, 어깨선 바로 아래(중점 + 0.05×shoulder width)의 얇은 상흉부 밴드(높이 0.20×shoulder width)로 만든다.

머리 anchor confidence `0.5` 이상인 귀가 한쪽만 있는 측면·3/4 입력은 같은 feature 안에서 머리 중심 x를 몸통 중심으로 사용한다. 몸통 ROI의 기준 y는 두 어깨의 높이 차가 `0.08` 이상이면 화면에서 더 아래인 실제 어깨를 사용해 위쪽 의자·헤드레스트 오검출을 피하고, 차이가 작으면 머리 중심 x에 가까운 어깨를 사용한다. 반면 머리 처짐 안전 조건은 높이 차와 무관하게 항상 머리 중심 x에 가까운 어깨 y로 계산해, 숙인 머리 반대편의 낮은 어깨가 간격을 부풀리지 않게 한다. scale은 두 어깨의 수평 간격이다. ROI 기준 어깨와 머리 인접 어깨는 confidence `0.35` 이상이어야 하며 반대쪽 어깨도 최소 추적 confidence `0.15`를 충족해야 한다. 반대쪽 confidence가 `0.35` 미만이면 그 점이 ROI 기준 어깨보다 위로 `0.08` 이상 튄 경우에만 의자·헤드레스트 오검출로 보고 허용한다. 이는 별도 시점 알고리즘이나 별도 baseline이 아니라 공통 ROI 기하의 제한된 fallback이며, 반대쪽 어깨가 사라지는 완전 측면까지 지원한다는 뜻은 아니다.

어깨 아래 깊은 위치는 노트북 내장 카메라의 전형 구도(어깨가 화면 하단)에서 항상 화면 밖으로 나가 프레임을 잃는다는 것이 2026-07-21~22 장치 검증으로 확인되어 채택하지 않는다. neck은 검출되면 진단 정보로 보존하지만 필수점은 아니다. ROI 크기는 고정 픽셀이 아니라 shoulder width 같은 신체 기준 길이에 비례시킨다.

ROI 경계에는 머리카락, 옷과 배경이 섞이기 쉽다. 가능한 경우 중심 영역을 사용한다. ROI 일부가 화면 밖이면 unit square로 clipping한 뒤 경계 접촉률과 depth 유효성을 다시 확인하고, 접촉률이 허용 상한을 넘거나 clipping 뒤 기하가 무효하면 프레임을 제외한다. 두 ROI가 과도하게 겹쳐도 제외한다. 보이지 않는 landmark를 고정 좌표나 임의의 대칭점으로 만들지 않는다.
...
3. 한 번의 버스트가 품질과 분산 조건을 통과하면 그 대표값과 변동 범위를 저장한다.
4. 품질 조건을 통과하지 못하면 10초 뒤 다시 수집하되 품질 판정까지 완료된 수집 시도는 총 3회까지만 실행한다. 절전이나 세션 중단으로 완료되지 못한 수집은 횟수에 넣지 않고 복귀 후 다시 시작한다. 카메라 사용 불가는 즉시 종료하며, 권한 거부는 권한 허용 안내로 구분한다.
...
한 번의 악화 증거는 `bad`가 아니라 악화 후보다. 최소 15초 뒤의 다음 점검도 악화일 때 `bad`로 전이한다. 자동 점검은 현재 캡처 시작 시각에서 설정된 주기가 지난 뒤 실행하며, 처리 완료 시각부터 다시 세거나 악화 확인용 별도 타이머를 만들지 않는다. 사용자가 `확인`으로 다음 점검을 앞당겨도 직전 캡처 시작 후 15초 하한과 같은 지속성 판정을 적용한다. 정상 복귀도 여러 번 확인한다.
