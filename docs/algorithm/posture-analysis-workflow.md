...
> 상태: 확정된 목표 설계. 구현은 이 문서의 역할 분담, 판단 순서와 제외 범위를 따른다.
...
목표는 Mac 내장 정면 RGB 카메라에서 사용자의 중립 자세보다 머리가 몸통 앞쪽으로 이동한 상태가 지속되는지 감지하는 것이다.

입력 조건은 다음과 같다.

- 한 명의 사용자가 카메라 앞에 앉은 정면 상반신 장면
- 머리, 목과 양쪽 어깨가 화면 안에 들어오는 구도
...
| Apple Vision 2D | RGB 프레임 | 상체 landmark와 관절별 confidence | 대상 선택, 머리·몸통 ROI, 품질 조건 | 전방 깊이와 최종 자세 |
...
flowchart TD
    ...
    NORMALIZE --> POSE["Vision 2D<br/>landmark·confidence"]
    ...
    BASELINE -- "아니요" --> RECALIBRATE["재보정 안내"]
    RECALIBRATE --> NOEVAL
    BASELINE -- "예" --> COMPARE["baseline 정상 범위와 비교"]
...
Vision landmark와 depth map은 같은 원본 프레임에 대응해야 한다. 두 경로의 회전, 미러링, crop 또는 resize가 다르면 잘못된 위치의 depth를 읽게 된다.

- 입력 orientation을 실제 카메라 방향과 일치시킨다.
- 화면 표시용 미러링과 분석 좌표계를 분리한다.
- Vision 좌표와 depth 좌표를 원본 이미지 좌표로 되돌릴 변환 정보를 유지한다.
...
품질 판단은 자세 feature를 만들기 전에 수행한다. 품질이 낮은 입력을 나쁜 자세로 해석하지 않는다.

| 확인 항목 | 통과 기준 | 실패 처리 |
|---|---|---|
| 대상 | 같은 사용자의 상반신을 구분할 수 있음 | 프레임 제외 |
| 필수 landmark | 머리 anchor, 목, 양쪽 어깨가 필요한 confidence를 충족 | 프레임 제외 |
...
## 4. Vision 2D landmark와 ROI

Vision 2D는 다음 세 역할만 담당한다.

1. 분석할 사용자의 상체 위치를 찾는다.
2. 머리, 목과 어깨를 기준으로 depth 값을 읽을 ROI를 만든다.
3. landmark confidence와 기하 조건으로 프레임의 평가 가능성을 판단한다.

머리 ROI는 nose, eyes, ears 중 품질을 충족하는 점을 기준으로 만든다. 몸통 ROI는 neck과 양쪽 shoulder를 기준으로 만든다. ROI 크기는 고정 픽셀이 아니라 shoulder width 같은 신체 기준 길이에 비례시킨다.
...
Vision 2D 관절 자체를 `good` 또는 `bad` 판정으로 해석하지 않는다. 좌표계와 API 세부사항은 [Apple Vision 로직 분석](apple-body-pose/analysis.md)에서 관리한다.
...
자세 분석기는 Vision 2D가 만든 ROI를 depth map에 정렬한 뒤 머리와 몸통 영역의 상대값만 읽는다. raw depth 숫자를 절대 임계와 직접 비교하지 않는다.
...
```mermaid
flowchart LR
    LANDMARK["Vision 2D landmark"] --> HEAD["머리 ROI"]
...
사용자 체형, 카메라 높이, 의자 위치와 촬영 거리의 영향을 줄이기 위해 안내된 중립 자세 baseline을 사용한다.

```mermaid
flowchart TD
    GUIDE["중립 자세 안내"] --> CAPTURE["여러 버스트 수집"]
    CAPTURE --> VALID{"유효하고 안정적인가"}
    VALID -- "아니요" --> RETRY["환경 조정 후 다시 시도"]
    RETRY --> CAPTURE
    VALID -- "예" --> ENOUGH{"충분한 표본인가"}
    ENOUGH -- "아니요" --> CAPTURE
    ENOUGH -- "예" --> BASE["중심값·정상 변동 범위 저장"]
    BASE --> READY["일상 분석 가능"]
```

baseline 생성 규칙은 다음과 같다.

1. 사용자가 중립 자세를 명시적으로 취한 보정 세션만 사용한다.
...
버스트 대표값과 baseline 중심값의 차이를 악화 방향으로 해석한다.

```text
delta = burst feature - baseline center
```

프레임별 feature의 방향을 이미 통일했으므로 양의 `delta`가 커질수록 baseline보다 악화된 방향을 뜻한다.
...
| baseline 정상 범위 안 | 정상 증거 | 중립 자세와 유사함 |
| 악화 경계를 넘음 | 악화 증거 | baseline보다 머리가 앞쪽으로 이동했을 가능성 |
...
한 번의 악화 증거는 `bad`가 아니라 악화 후보다. 악화 증거가 정해진 횟수나 시간 동안 이어질 때만 `bad`로 전이한다. 정상 복귀도 여러 번 확인한다.
...
| 결과 | 사용자 의미 | 알림 처리 |
|---|---|---|
| `good` | baseline 정상 범위의 자세가 확인됨 | 알림 없음 |
| `bad` | baseline보다 악화된 상태가 지속됨 | 새로 전이했을 때만 알림 후보 |
...
| 머리 또는 양쪽 어깨가 잘림 | 프레임 제외, 유효 프레임 부족 시 `noEval` |
| 큰 회전·가림·움직임 | 프레임 제외, 다른 알고리즘으로 전환하지 않음 |
| depth map과 landmark 좌표 불일치 | 분석 중단, `noEval` |
| 기준 ROI의 depth 범위 부족 | `noEval` |
| 버스트 분산 과다 | `noEval` |
| baseline 없음·불안정·촬영 조건 변경 | 재보정 안내 후 `noEval` |
...
| Vision 2D | landmark 반복성, 대상 선택 안정성, ROI 경계 오류 |
