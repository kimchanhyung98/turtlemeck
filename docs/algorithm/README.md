...
이 디렉터리는 현재 코드가 채택한 자세 분석 설계와 구현 근거를 관리한다.
상세 처리·판정 계약은 `posture-analysis-workflow.md`가 정본이며, 하위 문서는 채택한 모델·API·기하·기준 자세의 근거와 구현 경계를 설명한다.
...
| 4 | [apple-posenet/](apple-posenet/README.md) | 채택한 Apple Core ML 샘플 PoseNet의 모델·디코더·통합 경계 |
| 4 | [apple-body-pose/](apple-body-pose/README.md) | 보조 경로로 채택한 Apple Vision 2D API의 출력·좌표·품질 계약 |
| 4 | [pose-estimation/](pose-estimation/README.md) | 채택한 자세 추정 모델·기하·기준 자세·단안 한계의 구현 근거 |
| 4 | [../depth-estimation/](../depth-estimation/README.md) | 채택한 단안 깊이 모델과 자세 특성값 설계 근거 |

내용이 충돌하면 상세 규범 문서인 `posture-analysis-workflow.md`를 우선한다.
상위 `workflow.md`는 개론이므로 상세 판단을 새로 정의하지 않는다.
구현 계약을 바꿀 때는 상세 워크플로우와 코드를 함께 수정한 뒤 개론과 하위 근거 문서를 동기화한다.

## 알고리즘·방식별 공통 파일

독립적인 알고리즘이나 처리 방식은 `kebab-case` 디렉터리 하나로 묶고 다음 세 파일을 공통으로 사용한다.

| 파일명 | 필수 여부 | 역할 |
|---|---|---|
| `README.md` | 필수 | 상태·요약·입력/출력·간단한 처리 흐름·제품 적용 판단·문서 목록 |
| `analysis.md` | 필수 | 로직 구성, 단계별 처리, 수식·좌표계·API, 기술적 한계의 분석과 설명 |
| `references.md` | 필수 | 채택한 계약을 뒷받침하는 공식 문서·논문·로컬 구현 근거 |
| `validation.md` | 선택 | 데이터셋, 측정 프로토콜, 합격 기준, 검증 결과 |
| `comparison.md` | 선택 | 현재 선택을 설명하는 동일 기준 대안 비교 |
| `checklist.md` | 선택 | 구현·문서가 확정 설계와 일치하는지 확인하는 항목 |
| `related-<topic>.md` | 선택 | 현재 구현의 핵심 로직과 경계를 보충하는 근거 자료 |

`README.md`는 진입점이므로 상세 로직 분석이나 긴 출처 목록을 중복하지 않는다.
`analysis.md`와 `references.md`의 결론만 요약하고 링크한다.
선택 파일은 실제 내용이 있을 때만 만들며, 빈 형식을 맞추기 위해 생성하지 않는다.

인덱스 디렉터리, 규범 문서, 프로젝트 전체 체크리스트와 인덱스 직속 근거 모음(`etc/`)은 독립 알고리즘이 아니므로 이 세 파일 규칙의 예외다.
예외 여부는 상위 README의 문서 구성 표에 명시한다.

## 파일명 규칙

- 디렉터리와 선택 파일은 소문자 `kebab-case` 영문명을 사용한다.
- `README.md`만 대문자 고정 이름을 사용한다.
- 날짜, 버전, 상태(`draft`, `final`, `new`)를 파일명에 넣지 않는다.
- 필요한 상태는 문서 본문에 기록한다.
...
3. 제품 적용 판단: 채택·보조·검증 필요·제외 경계
4. 한계와 검증 상태
5. 문서 구성: 같은 디렉터리의 공통 파일 링크
...
- `comparison.md`: 동일한 평가 열을 쓰는 표로 작성하고 현재 선택과 대안 제외 이유를 명시한다.
...
| 검증 필요 | 채택 경로의 임계값·성능을 제품 데이터로 확정해야 함 |
| 미채택 | 비교 문서에서 현재 선택과 대조하는 후보이며 상세 조사는 활성 문서에서 관리하지 않음 |
| 제외 | 현재 구현에 넣지 않기로 확정한 경계 |
...
├── apple-body-pose/
│   ├── README.md                     # Apple Vision 2D 구현 근거 진입점
│   ├── analysis.md                   # Vision 2D body pose 분석
│   ├── references.md                 # Vision 2D 공식·구현 근거
│   └── checklist.md                  # 설계 적합성 검증
└── pose-estimation/
    ├── README.md                     # 채택한 자세 추정 설계 진입점
