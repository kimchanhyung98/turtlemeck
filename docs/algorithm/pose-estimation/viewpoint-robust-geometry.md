...
자세 추정·판정의 *각도 기하* 정확도에 집중한다(알림 정책은 범위 밖). 신뢰도 **[3-0]** = 3개 독립 검토자 만장일치. **핵심 결론: 단순 2D `atan2` 각은 버그를 고쳐도 부족하다 — 더 나은 기하는 body-frame 3D 각 + multi-feature 분리다.**
...
2. **시점 의존 + 단일 임계로는 분포가 겹친다 [3-0, 신규].** 비단조성을 고쳐도, *단일 2D 각도 임계*만으로 FHP와 정상을 가르기엔 부족하다.
...
## 2. 시점 강건성의 정공법 — body-centered canonical frame [3-0]
...
- ⚠️ **단순 hip/shoulder 정렬 정규화는 임의 회전에 강건하지 않다 [3-0].** 3DPCNet 원문: *"Rule-based normalizations that align the hips or shoulders are not robust to arbitrary rotations."*
...
## 4. Apple Vision 3D는 hip-rooted — 근접 착석에서 root가 불안정 [3-0]
...
- **Apple 3D API는 truncation/occlusion 처리·per-joint confidence를 문서화하지 않는다** [2-1]; 3D 점 타입에 per-joint confidence가 **없다**(2D 요청에는 있음 — 재확인). 샘플은 *"all limbs of the subject visible"* 를 권장(full body가 문서화된 정상 경로).
...
- **view-invariant embedding(Pr-VIPE, ECCV 2020/IJCV 2021)** 은 2D keypoint만으로 시점 불변 표현을 학습한다 [3-0]. 단 이는 **pose 유사도/검색**이지 *절대 FHP 각*이 아니며 multi-view 3D 학습데이터가 필요하다 → turtlemeck엔 *상대 good/bad 분류* 개념으로만 제한 적용.
- **반증되어 채택하지 않는 것:** "정면 각으로 sagittal CVA를 *예측* 가능"(0-3 기각), "정면 얼굴이미지 FHP 분류기"(K-FACE, 0.69 acc, 0-3 기각). 정면 단독 정량/분류는 근거 약함.
