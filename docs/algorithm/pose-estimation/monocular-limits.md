...
`turtlemeck`은 Mac 내장 **단일 정면 웹캠**을 쓴다. 이 구성의 구조적 한계와 대응을 정리한다. 신뢰도 **[high]** = 다수 1차 출처 합치.
...
## 2. 정면 단일 카메라로 전방머리 정량화 — 강한 근거 없음

정면 단독 FHP 정량화는 강한 근거가 없고 sagittal CVA를 대체하지 못한다. Molaeifar et al.(2021)이 보고한 moderate 상관은 정면 단독 proxy의 사용 가능성을 입증하지 못한다.
...
- 더 결정적으로, **고정확 landmark FHP 탐지 결과(예: Extra Tree Classifier 82.4% acc / 80.6% sens / 85.5% spec)는 전부 측면(side/sagittal) 뷰에서 얻어졌다.** 변별 feature가 acromion 기준 시상면 각·거리(NAH/ENA/PAH/EAH 등)라 **정면 카메라로는 계산조차 안 된다.** 즉 **정면 단독 FHP 탐지에는 검증된 고정확 선례가 존재하지 않는다.** (Yang et al. 2023, BMC Med Inform Decis Mak, 10.1186/s12911-023-02285-2)
- 결론: **정면 단독 정량화는 강한 근거가 없고 sagittal CVA를 대체하지 못한다.**

### 앱 함의 — 정면 정책
정책 *방향*은 보수적·추세 only·측면 유도다.
1. 정면에서는 전방머리 **심각도 점수를 단정하지 말 것.** baseline 대비 **상대 악화 추세**만 약한 신호로.
2. 강한 확신이 없으면 `noEval`(보수적 방침).
3. **측면/3-4 측면 착석 또는 카메라 측면 배치 유도**가 정공법(측면이 CVA 표준 기하 — [cva-and-fhp-metrics.md](cva-and-fhp-metrics.md) §1).
4. 정면 2D 보조 지표(머리 bbox 스케일·코-어깨 수직비·어깨폭 정규화 head offset)는 **미검증 가설**이다 — 실효성을 뒷받침하는 출처가 없다. "가능성 있는 신호"일 뿐 권고가 아니며, 채택 전 자체 데이터 검증이 필수다.
...
- 대응: **얼굴 yaw + 어깨 대칭/귀 가시성으로 viewpoint를 자동 분류**하고(앱의 `ViewpointClassifier`), viewpoint가 버스트 내에서 **안정될 때만** 판정. 자동 분류와 수동 설정이 충돌하면 자동 분류를 우선하거나 사용자에게 확인.

---

## 4. One-Euro(1€) filter — 실시간 스무딩 표준 [high]

- 원저자 논문(Casiez, Roussel, Vogel, **CHI 2012**, "A Simple Speed-based Low-pass Filter"):
...
- **대안 필터 대비:** 원 CHI 2012 벤치마크(약 1시간 커서 입력)에서 모든 필터를 **동일 jitter 수준으로 맞춘 뒤** 1€ filter가 **lag가 더 작았다** — *"[1e] has less lag across all speed intervals."* SEM도 1€=0.004로 가장 작고 Kalman·이동평균=0.015였으나(이중지수평활은 0.013), 이는 *동일 jitter 전제*에서의 lag 대리지표라 **"오차·lag 동시 우월"로 읽으면 과장**이다. 그래도 속도적응 cutoff 설계상 Kalman/EMA 대비 합리적 선택이라는 결론은 유지된다. 튜닝 규칙: *"if high speed lag is a problem, increase beta; if slow speed jitter is a problem, decrease fcmin."*
- 주의: 2-파라미터(최소 cutoff `fcmin`, 속도계수 `beta`) 튜닝 세부는 이번 검증 범위 밖 — 원논문 참조해 신호 채널별로 별도 튜닝.
- ⚠️ **알림 *결정* 레이어는 스무딩과 별개의 미해결 공백이다.** hysteresis/debounce/state-machine 기반 알림 전이(false-positive 억제)는 직접 뒷받침 출처가 없다 — per-channel 스무딩(1€)과 개념적으로 구분되며, HCI 알림/방해(interruption) 문헌·활동인식의 이중임계 hysteresis로 별도 조사가 필요하다.
...
1. **시계열 일관성(temporal consistency).** 연속 프레임의 2D keypoint 시퀀스에 장기 시간 의존성을 모델링(dilated TCN, VideoPose3D)하면 단일 프레임으로는 풀 수 없는 깊이 모호성이 완화된다 — *"3D poses in video can be effectively estimated with a fully convolutional model based on dilated temporal convolutions over 2D keypoints"* (VideoPose3D, arXiv:1811.11742). 일관된 bone length·느린 관절 변화가 모호성을 줄인다(Springer AI Review 2024).
   - **turtlemeck 적용:** 이미 있는 **버스트 누적 + 1€ 스무딩이 이 원리의 경량 구현**이다. 새 모델 없이, 알림 승격 전에 *프레임 간 일관성*을 활용하면 된다.
2. **해부학적/생체역학 prior.** 관절 관계 GCN·bone-length 일관성 같은 신체 구조 제약을 넣으면 다수 독립 baseline에서 정확도가 향상된다(PMC12031093; ACCV 2024 Bone Length Adjustment, arXiv:2410.20731, MPJPE 46.8→45.2mm).
...
- 정면 proxy (Work 2021) — 정면 proxy 타당성은 입증되지 않으며, "moderate 상관"은 proxy 검증 근거가 아님: <https://journals.sagepub.com/doi/abs/10.3233/WOR-213451>
