...
이 문서에서 확인한 출처들은 위 정의와 측면 측정 방식을 공통으로 사용한다. 이 결과를 모든 임상 프로토콜의 완전한 합의로 확대하지 않는다.

### 앱 적용
- C7과 tragus는 일반 웹캠 랜드마크에 직접 없다. Vision과 MediaPipe는 C7 극돌기를 점으로 주지 않는다. 앱은 CVA를 계산하지 않으며, neck과 shoulder는 depth ROI를 정의하는 anchor로만 사용한다.
...
확인한 문헌에서는 FHP cutoff 약 50°와 정상 경계 약 53°가 자주 쓰이지만, 어느 값도 엄밀히 검증된 단일 합의가 아니다.

### 제품 설계 함의
- 임상 CVA cutoff를 자체 relative-depth feature의 임계값으로 그대로 옮기지 않는다. 두 값은 기준점·기준선·관측 조건이 다르다.
- 출처별 임계 비합의와 증상↔무증상 중첩 때문에 개인 baseline 대비 상대 변화를 주신호로 둔다.
- 확인한 연구에서는 특정 어깨 각 하나로 군을 가르기에 분포 중첩이 있다. JMIR Formative 2024(e55476)는 어깨 각 분포의 겹침 때문에 그 각 하나만으로 두 범주를 구분하기 어렵다고 설명한다. 이 결과가 모든 각도 feature의 단독 사용을 원리적으로 부정하지는 않지만, 현재 설계에서는 각도 기반 절대 임계를 사용하지 않는다([시점별 자세 기하](related-viewpoint-geometry.md)).
...
### 앱 함의
- 사진 CVA와 방사선 지표의 `R²≈0.30`은 서로 다른 개념을 측정한다는 경고다. 이 값만으로 웹캠 feature의 수치적 상한을 추론할 수는 없으며, 독립적인 준거 타당도 검증 없이 임상 측정값으로 표시해서도 안 된다. 제품 표현은 "정밀 측정"이 아니라 "자세 습관 알림 신호"로 제한한다.
