...
## 0. 온디바이스 실측 (M1 Pro · macOS 15.7.2 · 노트북 웹캠 라이브 캡처 측정)

상체-only 3D 동작 여부를 실제 기기에서 측정한 결과다. 라이브 `CMSampleBuffer`(앱과 동일 입력 경로)로 캡처하고, 동일 프레임을 AI 시각 판독(ground truth)과 대조했다.
...
**코드 반영:** ① 3D 품질 게이트는 가려진 랜드마크(confidence 0)로 *신뢰도 0짜리 신호*를 만들지 않는다(미추적 proxy 제외 + 추적 임계 미만 noEval). ② fusion은 3D 입력이 실제 있을 때만 3D 분기를 평가(없으면 2D로 깔끔히 degrade).

### 0.1 "바른 자세 탐지"가 최우선 — 그리고 단안 2D의 노이즈 바닥
...
- **faceRoll**: 라이브 캡처에선 항상 0으로 나오나, **정지 PNG에선 삐딱(머리 ~30° 기울임) 시 roll=30.0°로 정확히 감지**된다(§0.2). 라이브/정지·환경 의존적이라 단정 불가 — body pose 실패 시 faceRoll을 삐딱 감지 보조로 쓰되, eye-line roll이 있으면 그것을 우선한다.
...
### 0.2 정면 거북목 클로즈업 = Vision body pose 완전 실패 (FaceTime HD · `analyze-image` raw 랜드마크 덤프)
...
- **단, 측면은 단일 정지 프레임에 한계가 있다**: 상체가 화면에 충분히 보여도 머리를 ~45° 돌린 정지 프레임(faceYaw=-45)에서 2D body 관절이 **전부 nil**이다(시점은 `threeQuarterLeft`로 분류되나 profileGeometry는 관절 부재로 noEval; bodyFrame3D만 88.3°=good 오판). 정자세조차 20프레임 중 일부만 2D 성공(간헐성). ⇒ Vision 2D body pose는 단일 정지에서 **정면 응시(yaw≈0)일 때 가장 안정**하고 머리 회전·근접에서 취약. '측면 유도'의 실효는 **라이브 버스트(여러 프레임 중 일부 성공 활용) + 몸 전체 프로파일**에서 검증해야 하며, 정지 프레임 단일 관측으로 단정하면 안 된다(라이브 경로 권장 — §0과 동일).
...
- **라이브 face 보조 거동**: ① **삐딱**은 faceRoll>28°로 baseline 무관 `bad`(5프레임 중 4, 1프레임은 faceYaw 0↔-45 요동으로 noEval→버스트가 흡수). ② **정자세**는 body 실패 세션에서도 baseline 주입 시 face proxy로 `good`(false-bad 0), 같은 baseline에서 **거북목 `bad`**. ③ **깊은 고개 숙임/젖힘(정수리·턱 뷰)**: face·body 둘 다 nil → noEval. 얼굴이 정면 평면을 크게 벗어나면 **face detector도 실패**한다 → 단안 정면 웹캠+face 기반의 탐지 한계(안전=false-good 아님, 단 미탐지). 적당한 숙임(얼굴 보임)은 face box y 하락으로 baseline 상대 탐지 가능.
