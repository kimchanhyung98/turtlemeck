...
- baseline은 사용자가 명시적으로 시작한 기준 자세 보정의 여러 프레임 분포로 만든다. 객관적 바름은 별도로 판정하지 않는다.
...
- `bad`는 저장된 baseline에서 충분히 이탈하거나, 머리는 감지됐지만 자세 때문에 정상을 확인할 수 없는 패턴이 지속될 때 반환한다.
- 프로젝트 자세 분석기가 relative depth feature에 baseline·품질·시간 일관성을 적용해 `good`·`bad`·`noEval`을 반환한다.
- 사람 없음·대상 모호성·모델 또는 depth 품질 실패·높은 분산은 `noEval`이다. 신뢰할 수 있는 머리가 있는 상태의 어깨 가림·상체 잘림·머리 기준 측면 기하로도 해소되지 않는 큰 회전·머리 처짐은 `noEval`과 구분하여 자세 기인 비정상 증거로 처리한다.
