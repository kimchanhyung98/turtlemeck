...
5. **제품 UI는 AI/ML 분석 방식만 노출한다.** 내부 회귀 테스트용 기하 방식은 남기되, 사용자 선택지는 `AI/ML 자동`, `Core ML Depth Anything`, `Apple Vision 3D 깊이차`, `Apple Vision 3D 신체축`으로 제한한다.
...
- 구현 진행 상태: 기본값을 `AI/ML 자동`으로 변경했고, UI는 ML 방식만 노출한다. `Resources/DepthAnythingV2SmallF16.mlpackage` 모델을 포함했다.
