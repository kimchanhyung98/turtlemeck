...
| 시점 band | 자동 선택 방식 | 근거 |
|---|---|---|
| `front` | `mlAuto` (현재 구현) | 정면은 depth 필요(리서치). 단 Core ML anchor가 없을 때도 기존 구현은 ML 자동 후보를 유지한다. Vision 3D fallback 리스크는 [`../_review.md`](../_review.md)에 보존 |
...
- 원 설계는 Vision 3D(`depthDelta`·`bodyFrame3D`)를 자동 라우팅 후보에서 제외하고 디버그 수동 선택으로만 유지하는 것이었다. 현재 코드는 front에서 `mlAuto`를 유지하므로 Vision 3D fallback이 살아 있다. 이 제품 판단은 확정 수정하지 않고 [`../_review.md`](../_review.md)에 남긴다.
...
2. **히스테리시스 K=2.** 시점 분류는 매 burst 수행하고, 새 band가 **2버스트 연속** 지배할 때만 라우팅 방식 전환. 플래핑 억제. 교차버스트 상태(`routeSelector`, `routedAlgorithm`)는 `CameraManager`의 serial `queue`에서만 접근한다.
...
- 구현됨: `ViewpointRouter`(순수 함수, 테스트 가능): `(band) → PostureAlgorithmID?`.
- 구현됨: `CameraManager`: `routedAlgorithm`·`routeSelector` 상태; `finishBurst`에서 지배 band 집계 + 히스테리시스 갱신; serial `queue`에서만 접근.
- 구현됨: `CaptureSnapshot`에 `effectiveAlgorithm` 추가 → `processSampleBuffer`와 `PosturePipeline.process(... algorithmOverride:)`가 이를 사용.
- 구현됨: `MenuView`: 방식 picker를 `if model.settings.debugEnabled` 안으로 이동(디버그 picker엔 `profileGeometry`/`frontProxy`도 노출해 측면 테스트 가능).
- `BurstProcessor`/`PostureStateMachine`: 전환 시 streak 리셋 훅.

## 엣지/주의

- `.unknown` 폴백: 직전 라우팅 방식 유지(교차버스트 메모리). 현재 초기값은 `mlAuto`.
...
- `ViewpointRouter`: band별 매핑 단위테스트(front→mlAuto, profile→profileGeometry, 3-4→profileGeometry, unknown→직전유지).
