...
OVERVIEW = (
    ...
    "사람 부재·모델·깊이 품질 같은 기술적 입력 부족은 판단을 보류하고, 신뢰 가능한 머리가 보이지만 자세 때문에 "
    "정상 확인이 어려운 패턴은 악화 증거로 구분한다. 어느 경우도 한 번만으로 주의 상태를 확정하지 않으며, "
    "좋지 않은 자세가 지속될 때만 알림을 보낸다. 운영 모드에서는 촬영 이미지를 저장하거나 외부로 전송하지 않는다. 메뉴 막대에서 현재 "
