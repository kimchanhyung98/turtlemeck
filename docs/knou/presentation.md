...
단일 이미지에는 움직임과 카메라 노이즈가 포함될 수 있어 한 번의 점검에서 여러 장, 최대 5장의 결과를 모아 대표값을 사용합니다. 상체가 잘렸거나 어깨가 충분히 보이지 않고 깊이 정보가 불안정한 경우에는 해당 결과를 제외합니다. 악화 결과도 연속해서 확인될 때 주의 상태로 바꿉니다. 판단하기 어려운 입력은 정상이나 주의로 억지 분류하지 않고 판단 보류로 처리합니다.
