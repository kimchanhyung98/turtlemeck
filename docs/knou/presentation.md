...
- 계속 촬영하거나 복잡한 장비를 사용하는 방식은 부담스러움

발표 대본:

컴퓨터 작업에 집중하면 머리가 앞으로 이동하거나 상체가 구부러져도 바로 알아차리기 어렵습니다. 일정 시간마다 울리는 일반 알림은 현재 자세가 좋은지와 관계없이 사용자를 방해합니다. 그래서 실제 자세 변화를 확인하되, 별도 장비나 지속적인 영상 저장 없이 사용할 수 있는 도구가 필요하다고 판단했습니다.
...
단일 이미지에는 움직임과 카메라 노이즈가 포함될 수 있어 3장에서 5장의 결과를 모아 대표값을 사용합니다. 상체가 잘렸거나 어깨가 충분히 보이지 않고 깊이 정보가 불안정한 경우에는 해당 결과를 제외합니다. 악화 결과도 연속해서 확인될 때 주의 상태로 바꿉니다. 판단하기 어려운 입력은 정상이나 주의로 억지 분류하지 않고 판단 보류로 처리합니다.
...
필요한 상체 위치와 깊이 품질을 얻지 못하면 `noEval`로 처리한다. 불확실한 입력을 나쁜 자세로 간주하지 않는다.
