...
문서의 사실 주장을 원출처(모델 카드, 논문, Apple 문서)와 실제 코드로 대조한 결과 수정이 필요한 오류는 발견되지 않았다.
"공식 자료로 확인한 사실"의 수치·라이선스·API 가용성 서술이 모두 원출처와 일치하고, 기술별 적용 상태 표는 7개 하위 문서의 자체 상태와 일치한다.
요약 플로우는 규범 문서(`docs/algorithm/posture-analysis-workflow.md`) 및 실제 코드와 정합한다.
상태 라벨 표현에 관한 참고 사항 1건만 기록한다.
...
`docs/depth-estimation/README.md`는 수치·라이선스·API 가용성·적용 상태·요약 플로우가 모두 원출처 및 코드와 일치하는 정확한 인덱스 문서다.
수정이 필요한 사실 오류는 없으며, apple-vision-depth 행의 상태 라벨 표기만 참고 사항으로 남긴다.
