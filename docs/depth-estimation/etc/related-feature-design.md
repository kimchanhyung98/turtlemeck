...
feature = direction * (head - torso) / max(scale, epsilon)
```

`d' = a·d + b`, `a > 0`인 전역 affine 변환에서는 분자와 IQR이 같은 scale을 받으므로 feature가 보존된다. 단, 이것은 전역 scale·shift만 제거하며 국소 depth 왜곡과 ROI 오류는 제거하지 못한다.
...
고정 pixel 크기, epsilon, 최소 면적, 분산 임계는 제품 해상도와 데이터로 정한다.
...
- 중립·악화 자세의 개인 내 분리도
- mean, median, percentile, rank 표현의 차이
