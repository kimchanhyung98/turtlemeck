...
Mac 내장 카메라의 고정된 시점에서 짧은 이미지 버스트를 사용해, 사용자의 중립 자세보다 머리가 몸통 앞쪽으로 이동한 상태가 지속되는지 판단하고 필요한 경우 알림을 보낸다.

분석 세션은 최소 15초 간격으로 실행하며, 한 세션에서 3~5장을 캡처한다.
...
baseline이 없으면 자세 판정을 시작할 수 없으므로, 최초 실행 시 기준 자세 보정을 먼저 진행한다. baseline이 없는 동안에는 `지금 점검`과 `일시정지`를 비활성화한다.

1. 최초 실행(또는 baseline 없음) 시 기준 자세 수집을 시작한다.
2. 한 번의 수집은 카메라를 1회 활성화해 버스트를 캡처한다. 유효한 기준 자세를 얻으면 baseline을 저장하고 정기 점검을 시작한다.
3. 수집에 실패하면 10초 대기 후 다시 수집하며, 수집은 총 3회까지만 실행한다.
4. 3회 모두 실패하면 최종 실패로 종료하고 동작을 중단한다.
   - 추가 점검을 예약하지 않으며, `지금 점검`과 `일시정지`는 비활성 상태를 유지한다.
   - 상태를 `보정 필요`로 표시하고 재보정 안내만 남긴다. 이후에는 사용자의 수동 실행이 필요하다.
5. 사용자가 `재보정`을 누르면 위 흐름을 다시 시작하고, 성공 시 정기 점검을 재개한다.
