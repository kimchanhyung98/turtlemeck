...
Mac 내장 정면 카메라의 짧은 이미지 버스트를 사용해, 사용자의 중립 자세보다 머리가 몸통 앞쪽으로 이동한 상태가 지속되는지 판단하고 필요한 경우 알림을 보낸다.

분석 세션은 최소 20초 간격으로 실행하며, 한 세션에서 3~5장을 캡처한다.
...
```mermaid
flowchart LR
    A["짧은 이미지<br/>버스트 캡처"] --> B["Vision 2D<br/>상체 위치·품질"]
...
| Apple Vision 2D | 사람과 상체 landmark 탐지, ROI와 입력 품질 결정 | 앞뒤 거리와 최종 자세 판정 |
...
flowchart TD
    ...
    ORIGINAL --> VISION["Vision 2D"]
    ORIGINAL --> DEPTH["Depth Anything V2 Small"]

    VISION --> CORE["공통 자세 분석"]
    DEPTH --> CORE
...
- 캡처한 원본 프레임
- Vision 2D landmark와 confidence
- Depth Anything V2 Small의 relative depth map
- 머리·몸통·기준 ROI
- 프레임별 feature와 버스트 대표값·분산
- baseline 중심값과 delta
...
- 한 세션에서 캡처하는 3~5장의 이미지는 `1`부터 번호를 붙이고 앞에 `0`을 채우지 않는다.
...
1. 3~5장의 짧은 이미지 버스트를 캡처한다.
2. Vision 2D로 머리, 목, 어깨 위치와 관절별 품질을 확인한다.
...
7. 악화 신호가 지속될 때만 `bad`를 확정하고 알림을 검토한다.

## 결과 의미

| 결과 | 의미 |
|---|---|
| `good` | 품질을 충족한 신호가 baseline 정상 범위에 있음 |
| `bad` | baseline보다 악화된 신호가 일정 시간 지속됨 |
...
| [Vision 2D 로직 분석](algorithm/apple-body-pose/analysis.md) | landmark, confidence, 좌표계와 ROI 역할 |
